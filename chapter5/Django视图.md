...
- 响应处理过程如下图： 
  ![这里写图片描述](https://img-blog.csdn.net/20171127225412061?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvdTAxMDUwNTA1OQ==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
...
```
http://www.itcast.cn/python/1/?i=1&p=new，只匹配“/python/1/”部分1
```

- 正则表达式非命名组，通过位置参数传递给视图

```
url(r'^([0-9]+)/$', views.detail, name='detail'),1
```

- 正则表达式命名组，通过关键字参数传递给视图，本例中关键字参数为id

```
url(r'^(?P<id>[0-9]+)/$', views.detail, name='detail'),1
```
...
匹配部分是：/booktest/1/
匹配过程：在settings.py中与“booktest/”成功，再用“1/”与booktest应用的urls匹配1234567
```
...
在urls.py中修改配置
from . import views1
url(r'^$', views1.index, name='index'),123456789
```
...
def getTest3(request):
    return render(request,'booktest/getTest3.html')123456
```
...
url(r'^getTest3/$', views.getTest3),123
```
...
def getTest2(request):
    ...
    b=request.GET['b']
    context={'a':a,'b':b}
    return render(request,'booktest/getTest2.html',context)12345
```
...
def getTest3(request):
    ...
    b=request.GET['b']
    context={'a':a,'b':b}
    return render(request,'booktest/getTest3.html',context)12345
```
...
def postTest1(request):
    return render(request,'booktest/postTest1.html')12
```

- 配置url

```
url(r'^postTest1$',views.postTest1)1
```
...
def postTest2(request):
    ...
    return render(request,'booktest/postTest2.html',context)1234567
```

- 配置url

```
url(r'^postTest2$',views.postTest2)1
```
...
url(r'^([0-9]+)/$', views1.index2, name='index2'),123456789
```

- 请求地址栏如图： 
  ![这里写图片描述](https://img-blog.csdn.net/20171127230101091?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvdTAxMDUwNTA1OQ==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
- 请求结果的地址栏如图： 
  ![这里写图片描述](https://img-blog.csdn.net/20171127230112628?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvdTAxMDUwNTA1OQ==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
- 推荐使用反向解析
...
def index(request):
    return HttpResponseRedirect(reverse('booktest:index2', args=(1,)))1234
```
...
def index(request):
    return render(request, 'booktest/index.html', {'h1': 'hello'})1234
```
...
def index(request):
    return redirect(reverse('booktest:index2'))12345
```
...
将settings.py中的DEBUG改为False12345678
```
...
禁用会话：删除上面指定的两个值，禁用会话将节省一些性能消耗123456
```
...
- 操作效果如下图： 
  ![这里写图片描述](https://img-blog.csdn.net/20171127225923363?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvdTAxMDUwNTA1OQ==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
- 在views.py文件中创建视图
...
def logout(request):
    ...
    return redirect(reverse('main:index'))1234567891011121314151617181920
```
...
def login_handle(request):
    ...
    return redirect(reverse('main:index'))1234567
```
...
```
SESSION_ENGINE='django.contrib.sessions.backends.db'1
```

- 基于缓存的会话：只存在本地内在中，如果丢失则不能找回，比数据库的方式读写更快

```
SESSION_ENGINE='django.contrib.sessions.backends.cache'1
```
